...
class OrderService
{
    ...
    public function buildDetailToken(Order $order): string
    {
        $createdAt = $order->created_at ? $order->created_at->getTimestamp() : 0;

        return hash_hmac(
            'sha256',
            implode('|', [$order->order_sn, $order->email, (string) $order->pay_id, (string) $createdAt]),
            $this->getOrderDetailSecret()
        );
    }
    ...
    public function hasOrderAccess(Order $order, ?Request $request = null): bool
    {
        if ($request && $request->filled('token')) {
            return hash_equals($this->buildDetailToken($order), (string) $request->input('token'));
        }

        if ($request && $request->filled('search_pwd') && !empty($order->search_pwd)) {
            return hash_equals((string) $order->search_pwd, (string) $request->input('search_pwd'));
        }
        ...
        return in_array($order->order_sn, $orderSNS, true);
    }

    private function getOrderDetailSecret(): string
    {
