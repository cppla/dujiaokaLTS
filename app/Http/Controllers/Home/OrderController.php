...
class OrderController extends BaseController
{
    ...
    public function bill(string $orderSN)
    {
        ...
        if (!$this->orderService->hasOrderAccess($order, request())) {
     ...
     * @link      http://utf8.hk/
     */
    public function checkOrderStatus(Request $request, string $orderSN)
    {
        $order = $this->orderService->detailOrderSN($orderSN);
        // 订单不存在或者已经过期
        ...
        if (!$this->orderService->hasOrderAccess($order, $request)) {
            return response()->json(['msg' => 'forbidden', 'code' => 400403], 403);
        }
        // 订单已经支付
     ...
     * @link      http://utf8.hk/
     */
    public function detailOrderSN(Request $request, string $orderSN)
    {
        $order = $this->orderService->detailOrderSN($orderSN);
        // 订单不存在或者已经过期
        if (!$order) {
    ...
    {
        return $this->detailOrderSN($request, $request->input('order_sn'));
    }
