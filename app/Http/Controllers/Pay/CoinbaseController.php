...
class CoinbaseController extends PayController
{
            ...
            switch ($payway) {
                ...
                default:
                    try {
                        ...
                        $redirect_url = $this->orderService->detailOrderUrl($this->order);  //同步地址
                        $cancel_url = $this->orderService->detailOrderUrl($this->order);  //同步地址
                        $config = [
    ...
    public function notifyUrl(Request $request)
    {
		...
		if ($sig === $sig2)
		{
			...
			if(in_array($return_status,$result_str)) {
