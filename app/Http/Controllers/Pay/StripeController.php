...
<script>
    ...
    public function returnUrl(Request $request)
    {
        ...
        if (!$this->validateOrderContextToken($cacheord, $request->input('context'))) {
            return redirect($this->orderService->detailOrderUrl($cacheord));
        }
        ...
        if ($source_object->status == 'chargeable' && $source_object->currency === 'cny' && (int) $source_object->amount === $expectedAmount) {
    ...
    public function check(Request $request)
    {
        ...
        if (!$cacheord) {
            ...
            $stripeUsdAmount = (int) $request->input('stripe_usd_amount', 0);
            if (!$this->validateOrderContextToken($cacheord, $request->input('context'), ['stripe_usd_amount' => $stripeUsdAmount])) {
                return 'fail';
            }
            $payGateway = $this->payService->detail($cacheord->pay_id);
            \Stripe\Stripe::setApiKey($payGateway -> merchant_pem);
            $source_object = \Stripe\Source::retrieve($data['source']);
            if ($source_object->status !== 'chargeable' || $source_object->currency !== 'usd' || (int) $source_object->amount !== $stripeUsdAmount) {
                return 'fail';
            }
            $charge = \Stripe\Charge::create([
                'amount' => $source_object->amount,
